...
if(typeof(dojo) != "undefined") {
  ...
  var createCommTiles = function() {
    var tileStyles = dojo.byId("tileStyles");
    if (!tileStyles) {
      var newTileStyles =
        '<style id="tileStyles">'+
    ...
    var newCommTiles = '<div id="commTiles" ' + (dojo.hasClass("viewControlTiles", "lotusTileOn") ? '' : 'class="lotusHidden"') + '>' + tiles + '</div>';
    dojo.place(newCommTiles, dojo.query("#lconn_communities_catalog_widgets_ResultsDisplayWidget_0 table")[0], "before");
  };

  var createTileToggle = function() {
    ...
    dojo.place(
      '<div id="viewControl" class="lotusViewControl lotusRight">' +
  ...
  // Wait for the page to be fully loaded, then render the flip card support
  waitFor(renderFlipCards(), "td a[dojoattachpoint='placeTitleLink']");
}
